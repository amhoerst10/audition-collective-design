...
| Any specific opening | **Must have a final audition date** (project owner, 2026-09-26). The final date is what expires a listing -- the nightly purge removes a listing only after its final audition date passes -- so an opening stored without one never expires and lingers on the board after the job is filled. Hunt for the date: linked audition PDFs, application portals, "auditions will be held on..." text. *Why this rule exists:* Alabama Symphony's Principal Trumpet was stored with no dates although the page said "Audition Date: September 23, 2026"; it stayed on the board after that date passed. |
| Source gives a single audition date (no separate rounds) | That date is the **final_audition**, never the preliminary. `preliminary_audition` is only for a distinct earlier round (prelims before semis/finals). A listing with a preliminary date but no final is always a data error (project owner, 2026-09-26: "how can that have a prelim, and not a final?"). *Why:* Waterloo-Cedar Falls' "Friday, Sept. 18" auditions were stored as preliminary dates, so the purge never expired them after Sept 18 passed. The nightly detector queues any prelim-without-final row for immediate review (failure_reason `prelim_no_final`). |
| Source genuinely gives no audition date | Allowed ONLY with the reason carried in the position name as a suffix, so members see why the date is blank: `(Contact to Schedule Live)`, `(Video / Audio Submission)`, `(Rolling)`, or `(Postponed to <what the page says>)` (e.g. Louisiana Philharmonic's "Principal Oboe Audition: Postponed to Spring 2027" -> `Principal Oboe (Postponed to Spring 2027)`). Never store an undated opening with a bare position name. Undated listings can't expire by date, so the nightly detector re-queues any whose `last_verified_at` is older than 60 days for a manual re-check (failure_reason `stale_undated`). |
